...
**Market-anchor gate — corrected-data PREVIEW (not the pre-registered test):** re-ran `scripts/market_anchor_gate.py` on the repaired DB. At **n=199** joined races: β=+0.029 (LR 0.09, NS, 90% CI [−0.11,+0.18] spans 0); OOS mean ΔLL **−0.0071/race** (90% CI [−0.0102,−0.0043], excludes 0 negative); SAR fold −0.048. **Verdict unchanged: NO-GO.** The correction halved the OOS drag (was −0.0164 at n=120 on corrupted labels) but did not flip sign or verdict — cleaning the data did not rescue the fundamental score.

**⏳ STANDING TRIGGER — pre-registered confirmatory re-run at n≈300 gate-joined races.** "n" = the gate's *joined* dataset (complete closing odds + comp_ex_val + no coupled + single winner), NOT total resolved. Currently **199 joined / 262 total** (join rate ~0.76 → ~13 more cards, roughly mid-Aug). The post-race pipeline (`r5_results_cli.py`) now prints the gate-joined count every card and a "🚦 GATE DUE" banner at ≥295. **Run `scripts/market_anchor_gate.py` exactly ONCE when first flagged** (repeated peeks at a pre-registered test inflate false positives), record the verdict here; if still not GO → ABANDON the market-anchored win-overlay program.
...
- **✅ Pre-registered model-vs-market test RUN (n=63): CONFIRMED.** Mean model-rank of winner 4.40 vs market-rank 3.13 (Δ +1.27); model under-ranks winner in 65% of races; winner in MODEL top-3 = 41% vs MARKET top-3 = 67%. This is the mechanism behind the SAR drag. **β-refit constraint:** close the winner-rank gap WITHOUT killing the value-divergence edge (came from diverging FROM the market, not matching it). Requires Harry ruling + version bump before any weight change — NOT a race-day fix. ⚠️ This diagnostic was run on WPS-bug-corrupted data (2026-07-06); re-run on the repaired DB before acting. The "rank-3 +17.4%" that motivated protecting the divergence edge is VOID (corrected +5.3%, single-payout).
...
- **HARD CONSTRAINT:** Class (+0.75) & FCI (+0.55) are the best winner/loser separators globally — do NOT cut weights globally. Any change must be SAR-scoped AND must not kill the value-divergence edge. Harry ruling + version bump required. ⚠️ Class/FCI attribution above (n=63/41) was fit on WPS-bug-corrupted labels — re-run on the repaired DB before any weight work. (Moot while ON ICE per the 07-06 NO-GO + n≥100 ruling.)
