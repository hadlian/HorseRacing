...
Dennis is a fully separate system. The following rules are non-negotiable:

- All Dennis code lives in `dennis/` at the repo root
- Separate SQLite DB: `dennis/dennis_results.db`
- **Read-only access** to:
...
- Field positions referenced from `r5_parser_v2.py` are copied as **literal integers** into `dennis/drf_to_csv.py`, never imported
...
dennis/
├── __init__.py              (empty)
├── dennis_engine.py         scoring engine
├── drf_to_csv.py            DRF → Dennis CSV converter
├── dennis_tracker.py        DB logging + result joining
├── dennis_backfill.py       one-shot backfill runner
├── dennis_compare.py        head-to-head report generator
├── dennis_cli.py            entry point
├── dennis_results.db        SQLite (auto-created)
├── csv/                     generated per-card CSVs
│   └── TRACK_YYYYMMDD.csv
└── reports/                 generated comparison Excel files
    └── dennis_vs_r5_NNraces_TIMESTAMP.xlsx
```

---

## 3. Database Schema (`dennis_results.db`)
...
CREATE TABLE picks (
    ...
    dennis_rank     INTEGER NOT NULL,     -- 1..N within race
...
-- Keys: dennis_version, backfill_complete, race_count, last_backfill_at
```
...
- Print a one-line decision log at the top of each generated CSV for all "resolve at build" fields:
  ...
  ```
- Output one CSV per card: `dennis/csv/TRACK_YYYYMMDD.csv`
- No imports from `r5_parser_v2.py`

---

## 7. Dennis Engine (`dennis_engine.py`)
...
Step 2: For each (track, date) card:
        ...
        c. Run drf_to_csv → dennis/csv/TRACK_YYYYMMDD.csv
           (Exception: if LRL0516.csv is confirmed BRIS Summary format, use it directly)
        d. Run dennis_engine.score_card(csv_path)
...
## 10. Comparison Report (`dennis_compare.py`)

Output: `dennis/reports/dennis_vs_r5_{N}races_{TIMESTAMP}.xlsx`

### Data Sources

- Dennis data: `dennis/dennis_results.db` (picks + results + category_picks tables)
...
Columns: Date, Track, Race, Surface, Distance, Race Type, Pace Scenario (from R5), R5 Pick, R5 Tier, R5 Score (comp), Dennis Pick, Dennis Tier, Dennis Composite, Dennis Consensus Count, Agreement (Y/N), Winner Name, Winner R5 Rank, Winner Dennis Rank, SP, R5 Top-Pick Win (0/1), Dennis Top-Pick Win (0/1), R5 ML, Dennis ML, R5 ROI Cell, Dennis ROI Cell
...
**Sheet 4 — Dennis Signals**

Diagnostic: does each Dennis-specific feature have predictive value?
...
One row per race where Dennis Rank 1 ≠ R5 Rank 1:
- Date, Track, Race, R5 Pick, R5 Score, Dennis Pick, Dennis Composite, Winner, Who Was Right, SP
...
Use Excel formulas throughout. Run `scripts/recalc.py dennis/reports/...xlsx` after generation and confirm zero formula errors before session ends.

---

## 11. CLI (`dennis_cli.py`)

```bash
python dennis/dennis_cli.py score <TRACK> <YYYYMMDD>     # score card to stdout, no DB write
python dennis/dennis_cli.py log <TRACK> <YYYYMMDD>       # score + write picks to DB
python dennis/dennis_cli.py results <TRACK> <YYYYMMDD>   # pull finish positions from r5 DB
python dennis/dennis_cli.py finalize <TRACK> <YYYYMMDD>  # mark late scratches
python dennis/dennis_cli.py backfill                     # one-shot 63-race backfill
python dennis/dennis_cli.py compare                      # generate comparison report
```
...
1. ✅ `dennis_results.db` contains **exactly 63 races** in results table
...
5. ✅ All 4 ROI numbers populated in Summary sheet (Dennis ML, Dennis SP, R5 ML, R5 SP)
6. ✅ `results/r5_results.db` pre/post SHA-256 checksums match (print both)
7. ✅ `git status` shows only `dennis/` files as new/modified
8. ✅ `grep -r "from Claude" dennis/` returns empty (no R5 imports)
