# Claude Code Prompt — Dennis v1 Build

**Paste this into a fresh Claude Code session. Companion document: `dennis_v1_spec.md` (v1.1).**

---

You are building **Dennis v1**, a parallel handicapping system that runs alongside R5 v3.4 for head-to-head comparison on the same 63-race universe. This is one focused session — build everything in `dennis/`, run the backfill, and produce the comparison report.

Read `dennis_v1_spec.md` in full before writing any code. It is the authoritative source.

---

## Critical isolation rules — non-negotiable

1. All Dennis code lives in `dennis/` at the repo root. Nothing else is created or modified.
...
3. **No imports from R5 modules** anywhere in `dennis/`. Copy field positions as literal integers.
...
### Step 2 — Create `dennis/` directory structure

Per spec Section 2. Create all files (empty stubs are fine at this stage).

### Step 3 — Build `dennis/drf_to_csv.py`
...
### Step 4 — Build `dennis/dennis_engine.py`
...
### Step 5 — Build `dennis/dennis_tracker.py`
...
### Step 6 — Build `dennis/dennis_backfill.py`
...
```bash
python dennis/dennis_backfill.py
```

Print the full output. If anything halts, report clearly and stop.

### Step 8 — Build `dennis/dennis_compare.py`
...
- Generate 4 ROI numbers: Dennis ML, Dennis SP, R5 ML, R5 SP
- Run `scripts/recalc.py` on output file
- **HALT if recalc returns any formula errors**

### Step 9 — Build `dennis/dennis_cli.py`
...
# Criterion 7: verify only dennis/ files modified
git status

# Criterion 8: verify no R5 imports
grep -r "from Claude" dennis/
grep -r "import r5" dennis/
```
...
Report path: dennis/reports/dennis_vs_r5_63races_TIMESTAMP.xlsx
